...
5. JSON comes back: `columns`, `returnedRowCount`, `truncated`, `truncationReasons`, `rows`.

`truncated` / `truncationReasons` cover: row cap (fetch limit is 201, then slice), per-field string cap (~2k chars), and total JSON byte budget (~100KB). `returnedRowCount` is the payload size — not the full query cardinality.
